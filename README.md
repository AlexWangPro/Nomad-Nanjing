...
- 重新设计点评弹窗：桌面双列、手机单列、统一输入框与星级卡片。
- 位置选择地图新增“定位到我”和“此处周边”。
- 定位或点击地图后可直接选择周边 POI，自动放置地点戳并写入地址。

- 地点可以通过高德搜索，也可以直接点击地图自行标注。
- 手动标注后可自行填写店名，并增加楼层、入口等位置备注。
- 已发布地点支持用户提交 1–5 星点评。
- 点评必须填写用户名和邮箱；邮箱只在管理员后台显示。
- 点评先进入审核队列，审核通过后才会公开并计入平均星级。
- 管理后台新增“点评审核”页面，可通过、拒绝或保留待审核。
